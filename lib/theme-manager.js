...
const schemaVersion = schemaUpdater.latestVersion; // version of the config schema used by this package for storing/loading themes
...
export default {
   ...
   * This will not look for updates, so loading a theme and then changing its
   * configuration will still report the loaded theme is still loaded.
   */
  isThemeLoaded(themeName) {
    let testTheme = atom.config.get(`${packageThemeStore}.themes.${themeName}`);

    return testTheme && testTheme.syntax.name === this.getCurrentSyntaxTheme().name && testTheme.ui.name === this.getCurrentUiTheme().name;
  },
