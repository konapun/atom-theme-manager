...
export default {
  ...
  updateSchema() {
    ...
    if (currentConfig.schemaVersion !== schemaVersion) {
      if (!currentConfig.schemaVersion) { // theme config was previously saved under "theme-manager"
        currentConfig = atom.config.get(legacyConfig);
      }

      currentConfig = schemaUpdater.updateSchema(currentConfig);
      atom.config.set(packageThemeStore, currentConfig);
      atom.config.set(`${packageThemeStore}.schemaVersion`, schemaVersion);
      atom.config.set(legacyConfig, undefined); // delete the old config as it's no longer needed
    }

    console.log(`${packageThemeStore} schema is current version`);
  },
